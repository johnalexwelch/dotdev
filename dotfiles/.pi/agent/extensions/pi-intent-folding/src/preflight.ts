...
export const INTENT_REQUIRED_PATTERNS: TaskPattern[] = [
  ...
  },
  {
    ...
    examples: ['refactor auth across 15 files', 'multi-file migration', 'update all controllers'],
  },
  {
    ...
    examples: ['comprehensive security audit', 'exhaustive performance analysis'],
  },
  {
...
export function checkPreflight(taskDescription: string, intentProvided: boolean): PreflightResult {
  ...
  return {
    ...
    matched_patterns: matchedPatterns,
    recommendation: 'Create an intent YAML with fold triggers and guards, then use: pi --intent path/to/intent.yaml',
  };
}
