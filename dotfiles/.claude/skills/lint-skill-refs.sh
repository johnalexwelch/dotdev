...
[ -d "$root" ] || { echo "skills root not found: $root" >&2; exit 2; }
...
for d in "$root"/*/; do
  ...
  refs="$( { grep -oE 'Load and (run|execute) `?[a-z][a-z0-9-]+/SKILL\.md' "$f" 2>/dev/null \
               | sed -E 's#^Load and (run|execute) `?##; s#/SKILL\.md$##'; \
             grep -oE 'follow `[a-z][a-z0-9-]+`' "$f" 2>/dev/null \
               | sed -E 's#^follow `##; s#`$##'; } | sort -u )"
  [ -z "$refs" ] && continue
...
[ "$dangling" -eq 0 ] && { echo "OK — no hollow references."; exit 0; } || exit 1
