...
- Record describe-pr evidence for the final gate: body file path, mode (`plan_backed`, `phase_run_backed`, or `issue_only`), issue refs discovered, phase evidence status, graphify usage, whether the body was applied to the PR, and deviation/new-finding counts when applicable.
...
- Verify that the `describe-pr` body file from Step 1 exists and was produced by `describe-pr` in this run. If it does not exist or was not generated by `describe-pr`, halt and rerun `describe-pr` first. **Draft PR creation is blocked until this file is confirmed.**
...
- Before pushing, check `git status`/diff for these known non-deliverable scratch filenames at repo root: `BRIEF.md`, `PROGRESS.md`, `DECISIONS.md`, `HUMAN-DECISIONS.md`, `MASTER-HANDOFF.md`. If any is tracked/staged AND was introduced on this branch (absent from `origin/<base>`), `git rm --cached` + gitignore it before push. Do not remove a matching file that already existed in the base branch. `HANDOFF.md`, `PRD.md`, `ISSUES.md` are skill-defined artifacts, not scratch — leave as-is.
- Push the branch to origin.
- If a PR exists, update the body with the file from `describe-pr` (not an ad-hoc hand-rolled summary or replacement body).
- If no PR exists, create one as draft with `gh pr create --draft --body-file <pr-body-path>`. Use **only** the body file produced by `describe-pr` in Step 1. `auto-merge-eligible` PRs are still created as draft until verification, review-comment resolution, CI, and issue reconciliation pass.
...
1. **Run repo verification commands** — execute the project's test/build/lint suite one final time against the PR branch. Check `package.json` scripts, `Makefile` targets, CI workflow definitions, or a Dockerfile lint/test target for the canonical commands — the exact set varies per repo, so derive it rather than hardcoding. **"Verify" means the project's *authoritative* gate, not a file-scoped subset.** File-scoped `ruff check`/`mypy`/`tsc` on only the files you touched — or a `pytest -k <name>` / single-file test run — is a proxy that CI does not run; a name/path-scoped test subset can pass while the full suite regresses (e.g. mocks whose arity a caller changed), or a fixture/schema change that a *different* test directory consumes: golden-SQL/`seed.yml` edits ripple into `tests/eval/` replay-harness consumers that read the raw fixture, so a `tests/unit/eval/`-only run is a false green (observed 2026-07-29: a P0a graded-facts change passed `tests/unit/eval/` but broke 7 `tests/eval/` harness tests on CI). **Lint and format commands must cover every directory CI covers — `src` AND `tests`, not `src` only:** a gate scoped to `src` passes locally while CI fails on `tests/` lint or format violations. The authoritative gate is the full set CI runs — typically a format *check* (e.g. `ruff format --check`, not just `ruff check`), the full or affected test suite, and any drift/consistency target (e.g. `make drift-check`). Absent a more specific CI-derived command set, the common Python default is `ruff check src tests && ruff format src tests && mypy src && pytest` — run the formatter (not `--check`) locally to fix issues before pushing; CI runs the check-only equivalent. This matters most **when the branch has not yet had a green CI run**: run the authoritative gate locally before the first push rather than discovering predictable failures (format, generated-file/env drift, doc-count, registry/attribution audits) on CI.
