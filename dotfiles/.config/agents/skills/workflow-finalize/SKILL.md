...
1. **Run repo verification commands** — execute the project's test/build/lint suite one final time against the PR branch. Check `package.json` scripts, `Makefile` targets, or CI workflow definitions for the canonical commands. **"Verify" means the project's *authoritative* gate, not a file-scoped subset.** File-scoped `ruff check`/`mypy`/`tsc` on only the files you touched is a proxy that CI does not run; the authoritative gate is the full set CI runs — typically a format *check* (e.g. `ruff format --check`, not just `ruff check`), the full or affected test suite, and any drift/consistency target (e.g. `make drift-check`). This matters most **when the branch has not yet had a green CI run**: run the authoritative gate locally before the first push rather than discovering predictable failures (format, generated-file/env drift, doc-count, registry/attribution audits) on CI.
