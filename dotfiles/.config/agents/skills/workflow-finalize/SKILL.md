...
Draft PRs stamp normally: the forge check accepts `pr-state` of `open` or `draft` and records the actual state in the stamp's checked fields (Phase 3 review F3, resolved), so the `human-only` default path needs no override. Any other forge-reported state (`merged`, `closed`, an error) still refuses the stamp — but note an already-merged or closed PR is usually invisible to the open-PR lookup and records `forge=no_pr` instead, and `no_pr` stamps **successfully** with the forge checks skipped; the refusal only guards states the forge actually reports. Never mark a human-only PR ready just to change its state — the stamp does not require it.
