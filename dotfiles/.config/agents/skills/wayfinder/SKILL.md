...
1. **Name the destination.** `/grill-with-docs` (lightweight) + `/domain-modeling` to pin down what this map finds its way to *and how it hands off* (to-prd — product or migration mode — / decision). Record the handoff target in the map's Notes. The destination fixes scope, so it's settled first.
