...
- workflow-review is `layer: judgment` — it returns lane files plus a synthesis verdict and never stamps (D-006 #12). This orchestrator records the gate: `ledger.sh stamp review --attest review_profile=<p> verdict=<v> lanes=<lane>=<run-scoped path>,...` — always pass run-scoped lane paths via `lanes=` (the kernel's bare `/tmp/<lane>-review.md` default can pick up a stale file from another run), confirm each lane file's `reviewed_sha` matches HEAD before stamping, and when the chosen profile escalates above a `fast` computed floor add `model_floor=opus` (the kernel's model floor tracks the computed floor, not the chosen profile). The kernel verifies the profile floor, lane files, and per-lane models; a stamp that won't write means the review didn't happen. Run-scoped paths apply to every shared multi-agent artifact, not just lanes (`../_docs/CONVENTIONS.md` § Run-scoped artifact paths).
