...
- workflow-review is `layer: judgment` — it returns lane files plus a synthesis verdict and never stamps (D-006 #12). This orchestrator records the gate: `ledger.sh stamp review --attest review_profile=<p> verdict=<v> [lanes=<lane>=<path>,...]` — the kernel verifies the profile floor, lane files, and per-lane models; a stamp that won't write means the review didn't happen.
