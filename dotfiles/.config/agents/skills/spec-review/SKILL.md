...
model: sonnet
reasoning: high
description: 'Review the diff between HEAD and a fixed point (commit, branch, tag, or merge-base) along two axes — Standards (does the diff follow this repo''s own documented coding conventions, using a Fowler code-smell baseline as fallback) and Spec (does the diff match the originating issue or PRD it claims to implement). Runs both axes as parallel, context-isolated sub-agents and reports them under separate headings. This is distinct from the built-in "code-review" skill: that one only checks correctness bugs and reuse/simplification/efficiency cleanups on the current diff, with no concept of repo standards-conformance or an originating spec — use this skill instead when the ask is about standards conformance, spec/PRD conformance, or reviewing a branch "since <ref>". Requires a git ref to diff against. NOT for reviewing a prose spec/PRD document itself (use clarity-review for that) — this reviews a code diff against its spec.'
---

## Contract

Consumes: a user-supplied fixed point (commit, branch, tag, or merge-base); the repo's documented coding standards (if any); the originating issue/PRD (via the issue tracker, an issue reference in the commits, or a spec file)
Produces: two independent findings reports under `## Standards` and `## Spec` headings, plus a one-line summary per axis
Requires: git; gh (or the configured issue tracker CLI) for spec lookup
Side effects: none — read-only; does not post comments, apply labels, or modify files
Human gates: fixed point confirmed (must resolve, diff must be non-empty) before spawning sub-agents; if no spec is found, ask the user before the Spec axis is skipped
...
1. Issue references in the commit messages (`#123`, `Closes #45`, GitLab `!67`, etc.) — fetch via the issue tracker described in `docs/agents/issue-tracker.md` (set up by `/setup-skills`).
2. A path or issue reference the user passed as an argument.
3. If the target issue carries a `## Parent` section (the `to-issues` issue template links child slices back to their parent PRD), fetch that parent PRD issue too — a `to-prd` PRD often states requirements the child issue only summarizes.
4. A PRD/spec file under `docs/`, `specs/`, or `.scratch/` matching the branch name or feature (for repos using the local-markdown issue tracker convention from `setup-skills`).
...
Anything in the repo that documents how code should be written, such as `CODING_STANDARDS.md`, `CONTRIBUTING.md`, or `CONTEXT.md`/ADRs in the area being touched.
...
- The path or fetched contents of the spec (and the parent PRD, if step 2.3 found one).
