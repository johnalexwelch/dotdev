...
| `review` | worktree verify; chosen profile ≥ `review-floor` — attest the bare word (`fast\|standard\|full`), never the printed `+security` token, which is refused; a `+security`-flagged floor adds a required security lane on top; every required lane file exists with `verdict:` line and an mtime no older than this run's init (a stale lane file from an earlier session is refused); **every lane verdict is tested, not merely recorded** — normalized (case-folded, trailing whitespace stripped — which also removes the trailing CR of a CRLF lane file) it must read `approve`, so `REQUEST_CHANGES`, `NEEDS_HUMAN`, an empty value, and any unrecognized token refuse the stamp (an allowlist against the contract token, not a denylist of rejection words — a typo like `aproved` refuses too); the **first** `verdict:` line wins, so a reviewer quoting `verdict: REQUEST_CHANGES` in prose cannot sink their own approving lane, and an `APPROVE` quoted below a rejecting header cannot rescue it; an **embedded** CR is not normalized away and refuses, so a mid-token `AP<CR>PROVE` cannot pass as an approval; a refused lane records no digest, its refusal reports the raw value, and the gate reads MISSING, not STALE; per-lane `model:` ≥ floor; digests recorded | verdict, review_profile, lanes, model_floor |
