...
| `finalize` | `check review` fresh; `git status --porcelain` empty; `verify-local` passed at HEAD; committed snapshot matches live state (`snapshot_current` — a snapshot-only commit is freshness-exempt, so out-of-band rewrites of the tracked record are caught here); via `forge.sh`: CI green, PR state, threads resolved (`no_pr` noted when no PR) | post_mortem, describe_pr, pr_number |
...
Typical workflows: workflow-router (init/reconcile/preflight), workflow-debug (diagnose/fix stamps), workflow-review (review-floor + review stamp), workflow-finalize (check review + finalize stamp + verify-local), run-backlog/execute-prd (read state for AFK monitoring)
