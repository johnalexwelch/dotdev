...
- **Graphify knowledge graph gate (conditional, before prompt-builder):** Inside the worktree, before running `prompt-builder`, check for an existing knowledge graph. If `graphify-out/graph.json` exists, run one focused `graphify query` for issue scope context (what modules/dependencies are relevant to this issue, what is the current architecture in the touched areas?) and record `graphify: queried` in your preflight output. If no graph exists, record `graphify: not_available_with_reason: <reason>`. Do not rebuild the graph — that is explicit `/graphify` work. Having graph context available informs sharper prompt-builder scoping and file selection.
