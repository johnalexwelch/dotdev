...
model: sonnet
reasoning: high
description: 'Harvest accumulated skill-improvement suggestions from session reflections into one standing ledger, cluster them by failure mode and cross-session frequency, decide what is worth doing, and dispatch approved items to write-a-skill. Use to process the skill backlog, review skill improvements, or turn reflections into skill changes.'
...
It plans and stops at approval, then hands each approved item to `write-a-skill` for implementation (the same seam as `workflow-feature` → `workflow-deliver`). It never edits a skill itself.
...
| Step 5: Dispatch To write-a-skill | conditional | pending | Per approved item |
...
Produces: an updated backlog ledger, a ranked decision queue, dispatched `write-a-skill` runs for approved items
Requires: nothing (stdlib/native — keeps `codex-compatible: true`); `write-a-skill` for implementation
...
Typical workflows: periodic skill retro; the consumer end of the session-insight → skill-backlog → write-a-skill pipeline
Pairs well with: session-insight (producer), write-a-skill (implements approved items), skill-evaluator (values uncertain items)
