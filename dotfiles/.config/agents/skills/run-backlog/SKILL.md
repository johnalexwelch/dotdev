...
2. For each completed dispatch:
   ...
   - PR lacks a complete `WORKFLOW_FINALIZE_GATE` block with valid `describe-pr` evidence (body_file from this run, mode, issues, phase_evidence, graphify, applied_to_pr) → flag `needs-human`; a PR URL, draft PR, green CI, or hand-written PR body alone does not satisfy finalization. Hand-written, copied, or ad-hoc PR bodies do not satisfy the describe-pr gate.
