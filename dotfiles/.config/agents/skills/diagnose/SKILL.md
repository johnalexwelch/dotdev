...
Pairs well with: tdd, improve-codebase-architecture

## Diagnosis Modes

- **quick**: Simple bugs, < 5 min. Skip hypothesis ranking. Single most-likely cause → verify → fix.
- **standard**: Default. Full Phase 1–6 loop. Hypothesis list + ranking.
- **deep**: Complex/intermittent bugs. Extended instrumentation. Multiple reproduction attempts. Correlation analysis.
- **production**: Live system. Read-only observation first. No speculative changes. Rollback plan required before any fix.
- **regression**: Known-good state exists. Binary search (git bisect or equivalent). Focus on delta between working/broken.
...
- **Minimized case**: [smallest reproduction]
- **Rejected hypotheses**: [what was ruled out and why]
- **Likely root cause**: [conclusion]
- **Evidence**: [logs, traces, tests proving the cause]
- **Proposed fix**: [approach]
- **Regression test needed**: [yes/no + description]
- **AFK-safe**: [yes/no — can an agent fix this unsupervised?]

## Routing Output

After diagnosis completes, emit exactly one routing recommendation:

- **direct-fix**: Root cause clear, fix is mechanical, low risk → proceed to implementation.
- **follow-up-issue**: Fix is non-trivial, needs its own issue/PR → create issue.
- **architecture-review**: Root cause is structural → invoke improve-codebase-architecture.
- **needs-human**: Insufficient context, ambiguous requirements, or business decision required → halt with artifact.
- **unsafe-for-afk**: Fix is clear but risk is too high for unsupervised execution → halt with artifact + fix plan.
