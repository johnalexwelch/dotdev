...
6. **gh-account-flip retry**: when running across personal and work GitHub accounts on the same host, `gh` occasionally resolves to the wrong account and fails with `Could not resolve to a Repository`. On any error matching that string, run `gh auth switch --user "${GH_PRIMARY_USER:-johnalexwelch}"` once (environment variable `GH_PRIMARY_USER` overrides the default primary user `johnalexwelch`; export per-environment or per-repo as needed), then retry the failed call with an explicit `--repo <owner>/<repo>` flag. If retry still fails, emit log line with `action=skipped reason=gh-auth-error` and continue to the next referenced issue.
...
- After `gh auth switch --user "${GH_PRIMARY_USER:-johnalexwelch}"`, the retry call with `--repo johnalexwelch/dotdev` also fails with `Could not resolve to a Repository`
...
### Interaction with drift-check #2 (revert detection)

The non-default-branch fallback and drift-check #2 (Issues closed incorrectly) have an intentional asymmetry: the fallback auto-closes issues without approval, but if a merge is later reverted, drift-check #2 (which requires approval to propose reopens) owns the decision to reopen. The fallback never re-scans already-processed PRs and treats a human reopen as authoritative — if a human has reopened a closed issue, the fallback will not auto-close it again. This separates the concerns: the fallback handles the immediate close when a non-default-branch PR merges; drift-check #2 handles the follow-up if that PR is reverted and requires human judgment about whether the issue should reopen. For details on this design decision, see decision-log entry DL-0017 in `docs/decision-log.md`.
