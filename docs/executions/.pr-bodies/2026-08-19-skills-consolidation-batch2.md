...
- Unverified pre-existing claims left as-is rather than guessed at: herdr's `# exit 1 on timeout` (needs a live pane and a deliberate timeout to confirm) and the spawn recipe's `--match ">"` prompt-readiness wait (any output containing `>` satisfies it — robustness, not security). Both predate this batch.
