...
| SB-099 | 2026-08-18 | 3 | 2026-08-06-ground-truth-before-locking-infra, 2026-08-11-3mf-multicolor-slicer-compatibility, 2026-08-10-litellm-pergamon-deployment | docs/agents/habits.md | One durable line: proxy (spec/prior/verbal/"mesh is valid"/config example) vs ground truth (repo, running state, target tool, live API) — verify in first iteration, before fixes or durable fan-out | high | new | implement | C42 | Ground-truth 2026-08-18: habits.md has no such line. Folds the 08-06 habits item + 08-11 "ground-truth first iteration" + 08-10 live-model-names lesson |
| SB-100 | 2026-08-18 | 1 | 2026-08-11-rowan-litellm-routing | i-have-adhd | Rule: when claiming a fix, show the verification command + output; never say "fixed" without evidence (3 untested "fixed" claims that session) | high | new | implement | C42 | Ground-truth 2026-08-18: rule absent from i-have-adhd/SKILL.md |
...
| SB-105 | 2026-08-18 | 2 | 2026-08-11-forgejo-mcp-ci-integration, 2026-08-10-litellm-rowan-migration | handoff | Env-assumption notes: when origin is self-hosted (Forgejo/Gitea) state that `gh` won't work + name the right tool; always include IP alongside LAN hostnames (resuming machine may lack /etc/hosts entry) | med | new | implement | C44 | Ground-truth 2026-08-18: neither note in handoff/SKILL.md. Doc guidance — not covered by d20's enforcement scope |
...
| SB-111 | 2026-08-18 | 1 | 2026-08-05-dbt-doc-skill-scope-creep | ponytail persona / docs/agents/habits.md | When lazy-ladder output is ENFORCEMENT (CI gate/lint/check blocking others), default opt-in single-target scope and confirm blast radius before repo-wide ("please do" approves the idea, not the blast radius) | med | new | implement | C47 | 3 build/rollback cycles that session; same lesson informed D-006 d18's warn-first rollout but the ponytail rule itself is unowned |
| SB-112 | 2026-08-18 | 1 | 2026-08-13-yagni-correctness-boundary | ponytail persona / docs/agents/habits.md | YAGNI applies to features/optimizations, NOT correctness guards (dedup, validation, trust-boundary error handling earn keep from line 1; "document the upgrade path" is a YAGNI smell) | high | new | implement | C47 | User-corrected deferral reversal; pair with SB-111 as one ponytail dispatch |
| SB-113 | 2026-08-18 | 1 | 2026-08-10-warren-pivot-insight | docs/agents/habits.md | Before accepting a tool/format/paradigm as a constraint, verify it's a real requirement, not "what the user happened to be using" (Obsidian assumption cost weeks) | high | new | implement | C48 | Ground-truth 2026-08-18: absent from habits.md |
...
### High-priority queue (8 items) — implement next
