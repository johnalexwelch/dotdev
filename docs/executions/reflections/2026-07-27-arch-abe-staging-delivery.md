...
_No user corrections — fully autonomous continuation. Findings below are Pass-B opportunities.*
...
_None. The merge-queue finding is an enhancement to `workflow-finalize`, not a new repeatable skill; it fails the "specific new workflow" bar as a standalone.*
