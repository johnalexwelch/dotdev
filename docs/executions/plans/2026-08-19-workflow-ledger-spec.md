...
**Indirection boundary (declared scope).** Rule 3 is a lexical tripwire, not a sandbox. Suppression that reaches a mutation through **name binding** — `./deploy.sh 2>/dev/null`, `f() { git push; }; f 2>/dev/null`, aliases — is **out of scope and fails open**. This is not closable lexically: `f() { git push origin main; }; f 2>/dev/null` (must block) and `bash test.sh 2>/dev/null && git push origin main` (must permit, batch #1) are lexically identical — mutation in one segment, suppression in another, no carrier token in either. Only name binding separates them. A predicate that blocks the first necessarily blocks every batch-#1 chain, which is the over-blocking item #1 exists to remove. Closing the class requires a tokenizer that tracks redirection scope. The load-bearing controls are the stamps and freshness rules, which do not depend on rule 3.
