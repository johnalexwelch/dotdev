...
| FIND-09 (SSH private key extractable from public history) | **Remediated 2026-08-18** | Key confirmed dead; clonable origin history verified clean (the 2026-07-09 rewrite held — this audit's local `git show 449613f:…` evidence succeeded only via a stale local `v0.1.0` tag, since fixed). Residual GitHub PR-ref/cached-blob persistence documented. See DL-0018 + `docs/audits/2026-08-18-find-09-remediation.md`. |
