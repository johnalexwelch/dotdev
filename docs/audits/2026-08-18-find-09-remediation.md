...
The local primary checkout scans identically post-cleanup. The known-benign header-string quotes are covered in this same delivery: a `private-key`-rule allowlist in `.gitleaks.toml` (which CI's secret-scan job uses), AND-scoped so that only *backtick-quoted* header strings on `docs/audits/*.md` lines are suppressed — real key material under that path still flags — plus refreshed `.secrets.baseline` entries. Accepted detect-secrets limitation: its `PrivateKeyDetector` hashes only the header string (identical hash for a quote and a real key), so for the two baselined audit docs gitleaks is the effective private-key control. The two scrub-secrets test fixtures remain in `dotfiles/.pi/agent/extensions/scrub-secrets/impact.test.ts` (lines 10/33) as intentional fake secrets; CI's range-based secret scan does not re-scan them unless that file changes.
