...
The local primary checkout scans identically post-cleanup. The known-benign header-string quotes are covered in this same delivery: a `private-key`-rule-scoped allowlist for `docs/audits/*.md` in `.gitleaks.toml` (which CI's secret-scan job uses) and refreshed `.secrets.baseline` entries. The two scrub-secrets test fixtures remain in `dotfiles/.pi/agent/extensions/scrub-secrets/impact.test.ts` (lines 10/33) as intentional fake secrets; CI's range-based secret scan does not re-scan them unless that file changes.
