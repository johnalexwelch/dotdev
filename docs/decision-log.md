...
## DL-0017 — Eval harness for the AI-response optimization loop (provisional)

**Date**: 2026-07-28
**Context**: Wayfinder map [#118](https://github.com/johnalexwelch/dotdev/issues/118) (daily AI-response optimization loop), research ticket [#119](https://github.com/johnalexwelch/dotdev/issues/119). The loop needs a quality signal; Alex named Harbor (harborframework.com) as a candidate and asked to survey the field. Criteria: local-first, no vendor lock, Claude/Anthropic support, scores agent/skill outputs (not just single prompts), machine-readable scores. Full survey: `docs/research/2026-07-28-eval-harness-options.md`. *(Correction 2026-07-28: "cheap to run manually" struck per Alex — run cost is not a constraint; entry below updated accordingly.)*
**Question**: Which eval harness scores the loop's AI-response quality?
**Decision**: **Provisional — no default winner ahead of #120; the pick is gated on the evaluation unit.** With run cost removed as a constraint, there is no cost thumb on the scale: if the unit is prompt→response pairs → **promptfoo** (native, config-driven, Anthropic-first, JSON output); if the unit is full agent task runs → **Inspect AI** (mature, MIT, local) as primary with **Harbor** a live contender — Harbor's Claude-Code container integration is the cleanest conceptual fit and its *only* remaining knock is maturity (Aug-2025-new). If Harbor proves stable in a spike, it wins the agent-run case. Final selection is gated on #120 (what gets evaluated), so this is a provisional direction, not a lock.
**Alternatives considered**: Harbor (watch-list contender, not rejected — best agent-eval fit; deferred on **maturity alone** now that container overhead is no longer a cost concern); DeepEval (kept as the LLM-judge-metrics layer, ridable on either backbone); Braintrust / LangSmith / Arize Phoenix (rejected — hosted, violates no-vendor-lock); Coder Eval / OpenEval / AgentEval Lab (watch-list — young/narrow).
**Tradeoffs accepted**: Selecting a backbone before #120 defines the corpus risks tooling for undefined requirements — mitigated by keeping this provisional and gating finalization on #120. promptfoo is now governed under OpenAI (Apache-2, still OSS) — acceptable given local self-host, but a re-check point if governance tightens.

## DL-0018 — iris-eval trustworthiness verdict for the AI-optimization loop

**Date**: 2026-07-28
**Context**: #124 (assess iris-eval) gates #120 (lock eval unit + dimensions). Read-only audit of `/Users/alexwelch/projects/agents/iris` (`staging`): `backend/src/iris/eval/` + `backend/eval/`. Full assessment: `docs/audits/2026-07-28-iris-eval-trustworthiness.md`. Follows DL-0017 (reuse IRIS's eval over promptfoo/Inspect/Harbor).
**Question**: Is iris-eval trustworthy enough to be the loop's quality signal now?
**Decision**: **Augment before adopting — NO-GO as-is, conditional GO as the harness.** Reuse iris-eval's code as the harness, but do not adopt its current default metric as the optimization objective until four gates land: (1) real non-null baseline (IRIS #945); (2) execution-equivalence as the *primary* SQL metric (demote token-overlap; drop the column-sum→0.8 boost that manufactures false passes); (3) an actually-run scored path, not CI's validate-only; (4) one consolidated 5-dim judge, model pinned, temperature=0, calibrated against ~10-15 human-scored outputs. Until then `pass_rate(token_overlap)` is a smoke test, never the objective. Reasons it fails now: null baseline (no improvement evidence), CI runs only `--validate` (zero automated signal), default SQL metric is lexical (wrong SQL can pass / correct-but-different can fail), and the LLM judge is unvalidated + gameable + has two divergent implementations.
**Alternatives considered**: **Trust as-is** — rejected: adopting an unvetted eval with a null baseline is direct Goodhart. **Don't-trust-at-all / re-open promptfoo/Inspect/Harbor (DL-0017)** — rejected: the harness code is a competent skeleton; the problems are fixable augmentations, not a rewrite. **Augment (chosen)**: keeps the reuse decision while gating the *signal* on concrete fixes.
**Tradeoffs accepted**: #120 stays blocked until at minimum the baseline (#945) and execution-primary metric land; the loop's first passes must treat quality as directional/smoke-test until the judge is human-calibrated. Coverage expansion (adversarial/ambiguous/unanswerable, retrieval-step tests) is recommended but not gating.

## DL-0019 — Loop scope = portable/cross-project; eval backbone = Inspect AI (supersedes DL-0017 for the loop)

**Date**: 2026-07-28
**Context**: Map [#118](https://github.com/johnalexwelch/dotdev/issues/118). After #124 (DL-0018) found iris-eval domain-locked to IRIS, Alex resolved the open loop-scope question: the loop must be a **reusable skill usable across dotdev projects**, not IRIS-only. That reframes the harness choice — reusing iris-eval couples the loop to one project. Alex also folded in Anthropic's "Demystifying evals for AI agents" framework, whose task/trial/scorer/transcript/outcome model + three-layer (output+execution / reasoning+planning / tool-use+action) framing describes **agent-run** evaluation, not prompt→response.
**Question**: What is the loop's eval backbone, given a portable/cross-project scope and an agent-run unit?
**Decision**: **Inspect AI (MIT, local) is the backbone.** Its abstractions map ~1:1 to Anthropic's agent-eval model (multi-scorer per task, code+model graders, transcript/outcome grading, pass@k/pass^k) and are the lower-total-code choice once tool-call + outcome/state grading are required. Supporting cast: **promptfoo** kept for the cheap prompt→response slice + quick pairwise (not the backbone); **DeepEval** as an optional judge-metrics library riding on top; **iris-eval demoted** to one project adapter (IRIS's reference-guided SQL grader behind Inspect's scorer interface — not deleted, execution-equivalence vs Redshift is genuine IRIS-only value); **Harbor** watch-list on maturity; **openai/evals** rejected (OpenAI-centric — OPENAI_API_KEY/Snowflake — and effectively maintenance-mode behind the OpenAI Dashboard). This **supersedes DL-0017** for the loop: DL-0017's "reuse IRIS's eval over promptfoo/Inspect/Harbor" no longer holds once the loop must be portable.
**Alternatives considered**: **Loop signal = iris-eval (DL-0017/DL-0018 reuse path)** — rejected: not portable, scores only NL→SQL. **Build our own harness** — rejected: YAGNI, reinvents Inspect/promptfoo. **promptfoo as backbone** — rejected as backbone: strains on outcome/state/transcript/tool-call grading that the agent-run unit needs (would need custom providers for what Inspect does natively); kept for the prompt slice. **Hosted (Braintrust/LangSmith/Phoenix)** — rejected: vendor lock.
**Tradeoffs accepted**: Inspect needs Python solvers/scorers per eval (more code than a promptfoo YAML), accepted because the unit is agent runs. Final lock still gated on #120 defining the actual unit/corpus — if the real unit turns out to be single-shot prompt→response, promptfoo leads instead. Evidence tier for tool capabilities is *inferred* (web research, not hands-on); a one-task Inspect spike should confirm before hard-locking.

## DL-0020 — Grader architecture, LLM-judge scoring, and top-5 per-run cap

**Date**: 2026-07-28
**Context**: Map [#118](https://github.com/johnalexwelch/dotdev/issues/118), following DL-0019 and Anthropic's eval framework. Defines *how* the loop scores and *how much* it emits, so #120 can lock the unit against a concrete grading design.
**Question**: Which graders, which LLM-as-judge scoring setup, how many judges, which layers, and how many proposals per run?
**Decision**:

- **Graders (all three, each one job)**: code-based first (outcome/state checks, tool-call verification, transcript metrics — fast/objective); LLM-judge where nuance is needed; human graders reserved to **calibrate the LLM judges** (~10-15 spot-checks, then occasional), not for volume. Anthropic's rule: deterministic where possible, LLM where necessary, human judiciously.
- **LLM-judge scoring**: **pairwise (candidate vs baseline) is the loop's primary mode** — the loop's real question is "did this change beat baseline," and judges are more reliable comparing than scoring absolutely; **pointwise rubric** for per-dimension tracking/deltas (treated as noisier); **reference-guided** wherever a gold answer exists (e.g. iris-eval SQL). **One isolated judge per dimension**, never one judge scoring all dims (iris-eval's flaw); give the judge an "Unknown" way out to curb hallucination.
- **Consensus**: 1 judge/dimension to start; add 3-judge median **only** where a spike measures high variance — not speculatively.
- **Layers**: grade **output quality + outcome** primarily; **tool usage** lightly (right tools/params, **not** rigid call sequence — Anthropic calls sequence-checking too brittle; "grade what it produced, not the path"); **reasoning/planning** only indirectly via outcome unless a specific failure mode warrants it.
- **Corpus**: a **private 20-50 task suite**, split **capability** (low pass rate, hill to climb) vs **regression** (~100%, catch backsliding); public benchmarks (SWE-bench, τ2-bench, etc.) are a separate model-selection concern, not the daily loop. **[Corrected 2026-07-28 per DL-0021]:** "from real dotdev failures" reflected DL-0019's portable framing, which DL-0021 superseded — the v1 corpus is **IRIS's golden set** (`backend/eval/golden_sql/seed.yml`, in `classdojo/iris`), not dotdev failures. **"Private"** = the **held-out split the optimizer never trains on** (DL-0023), not private-from-iris.
- **Report cap**: each run emits **at most ~5 highest-impact proposed changes** (bounds combinatorial judge spend *and* human review load — the loop's true bottleneck is human attention, not compute).
**Alternatives considered**: one judge for all dimensions (rejected — iris-eval's exact flaw, conflates signals); pointwise-only scoring (rejected — absolute LLM scores drift/miscalibrate; pairwise is more robust for the beat-baseline question); rigid tool-sequence grading (rejected per Anthropic — punishes valid creative paths); grade all three layers richly day one (rejected — cathedral; outcome+light-tool-use is 80/20); unbounded proposal list per run (rejected — swamps the human approver).
**Tradeoffs accepted**: pairwise costs O(candidates×baseline) comparisons — the top-5 cap is partly what keeps that bounded; per-dimension isolated judges multiply calls vs one blended call — accepted for signal cleanliness. Reasoning/planning quality is under-measured by design; revisit if outcome-grading misses a real reasoning failure mode.

## DL-0021 — v1 = thin owned driver + cherry-picked iris-eval, grading analysis+findings not just SQL (supersedes DL-0019)

**Date**: 2026-07-28
**Context**: Map [#118](https://github.com/johnalexwelch/dotdev/issues/118). Independent specialist audit (correctness / build-vs-adopt / risk) of the *actual* iris-eval code, plus two user corrections: (1) the system-under-test is the **IRIS agent** (dotdev is only the loop's home), and (2) the product is IRIS's **analysis + findings**, not just its SQL. Ground truth I verified directly: iris-eval has **332 passing unit tests** (so "completely vibecoded/untested" is refuted), but three dangerous defaults are real — `comparator.py:195` column-sum boost forces `row_score≥0.8`, `engine.py:20` defaults to token-overlap≥0.8 (Goodhart), and `answer_judge.py` scores accuracy **without seeing executed data**. The golden set carries curated `expected_answer_facts` per pair (reference anchor for findings). This supersedes DL-0019 (Inspect backbone) — Inspect duplicates unavoidable wiring cost and rebuilds domain logic iris-eval already has and tests.
**Question**: What is the v1 eval foundation for a loop that tests+optimizes the IRIS analyst agent's SQL *and* analysis/findings?
**Decision**: **Thin owned driver in dotdev + cherry-pick only iris-eval's proven pieces. No wholesale adopt of iris-eval, no Inspect, no from-scratch rewrite.**

- **Eval unit** = an IRIS analysis response: SQL + executed data + **findings/analysis**.
- **3 grading layers, mapped by trust**: (1) **data correctness** — reuse the execution-equivalence comparator (proven, 332 tests); (2) **findings grounding/faithfulness** — numeric grounding (findings' numbers vs executed rows = code) + a **data-grounded, reference-anchored** fact-coverage judge anchored on `expected_answer_facts` and **shown the real data** (this is the non-negotiable anti-hallucination trust boundary); (3) **analysis quality** — calibrated **pairwise-vs-baseline** judge, one isolated dimension per call, **advisory-only until human κ ≥ 0.7** (Goodhart guard — never drives an auto-proposal before calibration).
- **Import (proven/tested)**: execution comparator + golden-set loader (with `expected_answer_facts`).
- **Own (small/correct, ~80-120 LOC)**: driver + the grounded findings judge + numeric-grounding check — own the judgment logic because trust is *earned via calibration*, not inherited.
- **Fix**: remove column-sum boost (~5 lines); don't use token-overlap as the metric.
- **Baseline**: capture manually from a known-good IRIS SHA — don't wait on IRIS #945 automation.
- **Skip/defer** (tracked on the #118 roadmap): iris-eval's `answer_judge`/`llm_grader`/composite/CLI (untrusted judge surface), lane/side-door + tool-use "how-derived" checks, Inspect AI (target #2), multi-turn, auto mode.
**Alternatives considered**: **Augment iris-eval wholesale** (my earlier lean) — rejected: inherits the untrusted judge/token-overlap/composite surface Alex is right to distrust. **Adopt Inspect/promptfoo backbone** (DL-0019, arbiter's anchored pick) — rejected for v1: pays the same unavoidable wiring cost *plus* rebuilds the domain SQL comparator + golden loading; its runner win doesn't beat that for a single target; the arbiter reached it only by circularly trusting DL-0019. **Clean-room everything** — rejected: throws away a 332-test execution comparator and an irreplaceable curated golden set. **SQL-only v1 (skip judge)** — rejected: doesn't test the actual product (analysis/findings).
**Tradeoffs accepted**: v1 adds LLM-judge cost — bounded by the top-5/run cap (DL-0020) + isolated-dimension + pairwise. We own+maintain a small judge instead of importing one. Analysis-quality scores are under-trusted (advisory) until calibrated — accepted; correctness (layers 1-2) gates, taste (layer 3) doesn't, until κ proves it. Flip-back trigger: if iris-eval's comparator API / IRIS event protocol proves unstable, write our own ~200 LOC execution comparator instead of importing.

## DL-0022 — IRIS eval v1 dimensions + thresholds locked (#120), by 3-specialist consensus

**Date**: 2026-07-28
**Context**: Map [#118](https://github.com/johnalexwelch/dotdev/issues/118), ticket #120, under DL-0021's 3-layer architecture. Alex directed: accept the recommended selections but have specialist subagents review, looping until consensus. Ran a 3-round taskflow (`eval-selection-consensus`): a parallel panel — eval-methodology (critic), IRIS-domain (analyst), skeptic (risk-reviewer) — reviewing each round, with a plan-arbiter reduce emitting `{consensus, revised_selections, critique}`. Reached **consensus (critique: none) on round 3**.
**Question**: What are the concrete layer-3 analysis-quality dimensions and per-layer pass thresholds for the IRIS response eval?
**Decision**: Locked in [`docs/design/iris-eval-v1-selections.md`](design/iris-eval-v1-selections.md). Headlines:

- **Layer-3 dims (advisory until human agreement ≥0.7)**: Sufficiency, Completeness, Insight — each with behavioral anchors; `citation_accuracy` dropped (dead ~0.5 weight).
- **L1 data correctness (gate)**: order-independent set equality + 1% float tol + alias normalization; no regression vs baseline.
- **L2a findings grounding (immediate gate, temp=0, trust boundary)**: zero fabricated numeric claims (precise definition: not literal in rows AND not one-arith-op-derivable within tolerance, OR attribution-incorrect via tuple-joint check); tolerance 2%/10% with an approx-language cap of ≤3; affirmative ≥1 grounded required-fact claim (anti-omission); binary required-facts gate; fabricated references folded into attribution; failure-type tagging.
- **L2b fact-coverage (advisory until ≥0.6)**: threshold = P10 of the baseline distribution computed with the *new* grounded judge (not iris-eval's legacy judge).
- **L3 quality (advisory)**: pairwise-vs-baseline per dim, excluded from aggregates until calibrated.
- **Calibration**: N≥30 purpose-built (flat/ambiguous incl. weekend-dip, signal-present, tie anchors); humans score pairwise; report Gwet's AC1 + Cohen's κ with 95% CI; gate L3 at κ≥0.7 AND CI_lower≥0.55, L2b ≥0.6.
- **New P0 prerequisites**: `expected_answer_facts` audit (incl. negative test + authoring quantitative findings-layer facts) → then baseline capture (sequenced after audit, new judge) → P1 purpose-built calibration set.
**Alternatives considered (rejected during review)**: fact_coverage as an 0.8 hard gate (rejected — it's an LLM judge, must be advisory-until-calibrated → split to L2b); Sufficiency requiring "commit to interpretation" (rejected — overlapped Insight, penalized valid restraint on lookup questions); requiring table/column citations in findings (rejected — prose for non-technical users; only fabricated references penalized); 1%/5% tolerance (rejected — misfired on natural prose rounding → 2%/10%); N=10-15 calibration (rejected — CI too wide → N≥30 + CI_lower gate).
**Tradeoffs accepted**: v1 can't produce a *trusted* score until the P0 facts audit + baseline land (design is final, wiring waits). L3 stays out of ranking until calibrated. More upfront calibration rigor (N≥30, pairwise, AC1+κ+CI) in exchange for a defensible quality signal.

## DL-0023 — Candidate-generator (loop "explore" half) design locked, by 4-round specialist consensus

**Date**: 2026-07-28
**Context**: Map [#118](https://github.com/johnalexwelch/dotdev/issues/118). The eval (DL-0022) is the loop's "measure" half; #118 flagged the "explore/candidate-generator half" as undesigned. Same grill process as DL-0022: a 4-round `candidate-generator-consensus` taskflow — parallel panel (eval-methodology/critic, IRIS-domain/analyst, skeptic/risk) reviewing each round, plan-arbiter reduce emitting `{consensus, revised_selections, critique}`. **Consensus (critique: none) on round 4**, all three specialists explicitly verdicting "no blocking design holes remain."
**Question**: How does the loop generate, score, rank, and emit candidate changes without over-fitting fixtures, reward-hacking the eval, or silently regressing the agent?
**Decision**: Locked in [`docs/design/iris-loop-candidate-generator-v1.md`](design/iris-loop-candidate-generator-v1.md). Headlines:

- **Candidate types (5)** with blast-radius tiers: prompt/instruction, reflections/habits (scoped), tool schema, retrieval-corpus (EVAL-INVISIBLE → human route), routing/findings-pipeline code. Eval fixtures/judge **excluded** (meta firewall).
- **Input = failure cluster** by (L2a tag + domain + lane); L3 excluded until κ≥0.7; query-pattern-tag taxonomy pre-enumerated.
- **Anti-overfit/reward-hacking**: train/holdout split within class (only counts if holdout moves), N+7 quarantine, generator≠judge model, sentinel loop-health candidate, reviewer "fluency vs grounding" checklist. Explicit residual: ~25-pair single-author corpus makes holdout weak → **human approval is the primary defense**.
- **Scoring**: pairwise-vs-baseline across whole suite; severity L1=3/L2a=2/L2b=1 (L3 excluded); **permutation p-values + Benjamini-Hochberg FDR q=0.05 over M_total** (no 2σ — net is discrete at N=25); hard **regression gate** on L1/L2a/L2b-floor; L3 drop = flag only.
- **Emission**: top-5 (DL-0020) with a confounder card (permutation p, FDR q, blast tier, cross-lane note, interpreted flags) → human approval → route. Fixture-gap notes route to the P0 audit queue, off the top-5 budget.
- **Baseline/drift**: refresh on model bump or 5 approved; delta-vs-baseline CUMULATIVE-REGRESSION flag; pinned model version + temp=0 prereqs.
**Alternatives considered (rejected during review)**: per-case generation (rejected — overfits one fixture → cluster-by-mode); 2σ significance threshold (rejected — assumes normality, invalid on discrete net at N=25 → permutation + BH); BH over M_holdout (rejected — ~2× FDR inflation → M_total with p=1.0 for holdout-failers); LOW-tier net≥0 (rejected — allows no-op emission → net>0 all tiers); generator editing eval fixtures (rejected — meta conflict of interest → firewall + fixture-gap queue); trusting the holdout split as the primary reward-hacking guard (rejected on a single-author 25-pair corpus → human gate primary).
**Tradeoffs accepted**: metric-disambiguation failures are an EVAL-INVISIBLE v1 blind spot (retrieval bypassed in fixtures); holdout is a weak secondary signal at current corpus size; regression attribution is batch-granular (≤5 candidates), not per-candidate.
**Open (human input, non-blocking)**: fixture-gap queue owner beyond v1; fixture-author diversification; v1 acceptance of the metric-disambiguation blind spot; run frequency (daily vs manual).

## DL-0024 — Loop-driver + morning report + routing + state (M3) design locked, by 5-round specialist consensus

**Date**: 2026-07-28
**Context**: Map [#118](https://github.com/johnalexwelch/dotdev/issues/118). Third and final design surface for the loop: measure=DL-0022 (eval), explore=DL-0023 (candidate-generator), and this M3 = the driver that assembles the ranked top-5 into a morning report, gates on human approval, routes approved items to IRIS, and persists loop state. Same grill: a 5-round `m3-loopdriver-consensus` taskflow — parallel panel (systems-coherence/critic, IRIS-domain/analyst, skeptic/risk) each round, plan-arbiter reduce. **Consensus (critique: none) on round 5**, all three specialists approving; architecture was stable from round 2, rounds 3-4 hardened operational mechanics, round 5 confirmed no blocking hole under an explicit convergence criterion (blocking = silent data loss / silent wrong-routing / approval bypass / counter-baseline corruption).
**Question**: How does the loop present candidates, secure a genuine (non-rubber-stamp) human decision, route approved changes to the real IRIS repo, and persist state across manual daily runs without double-applying, corrupting the change-counter, or leaking secrets?
**Decision**: Locked in [`docs/design/iris-loop-driver-report-routing-v1.md`](design/iris-loop-driver-report-routing-v1.md). Headlines:

- **State** = committed JSON on a dedicated `loop-state` branch in dotdev (DL-0011 blocks main pushes), SHA-256 checksum + refuse-on-mismatch, single-operator + file lock.
- **Approval** = CLI (`loop approve/reject/edit`), **DEFAULT=REJECT** (no action = not routed), no batch-approve, no `--auto-approve` in v1; the GitHub report issue is read-only. Routing executes inside `loop approve`.
- **Routing** = one artifact per candidate into `classdojo/iris` (the SUT); Types 1-3 → PRs at paths from `iris-*-targets.yaml`, Types 4-5 + fixture-gap → issues (PENDING-IMPLEMENTATION). Two-phase write + crash-resume probe + path-existence validation.
- **Reconcile** auto-runs first each run: PR landing dedups on `{repo, mergeCommit.oid}` (squash-safe); issue landing requires **closed + `loop-landed` label** or explicit `loop mark-landed` (blocks won't-fix closures from false counter increments).
- **Baseline loop** closes: change-counter hits threshold → propose refresh (never auto) → capture resets quarantine + re-quarantines holdout (DL-0023 §E); cumulative-regression attribution to the ≤5-candidate batch.
- **Secrets** never enter state/reports/PRs; preflight access check before scoring; deny-list scrubbing.
- **Auto mode deferred** to a later slice; v1 fully human-gated.
