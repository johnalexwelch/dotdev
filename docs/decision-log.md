...
**Date**: 2026-07-28
**Context**: Wayfinder map [#118](https://github.com/johnalexwelch/dotdev/issues/118) (daily AI-response optimization loop), research ticket [#119](https://github.com/johnalexwelch/dotdev/issues/119). The loop needs a quality signal; Alex named Harbor (harborframework.com) as a candidate and asked to survey the field. Criteria: local-first, no vendor lock, Claude/Anthropic support, cheap manual runs, scores agent/skill outputs (not just single prompts), machine-readable scores. Full survey: `docs/research/2026-07-28-eval-harness-options.md`.
**Question**: Which eval harness scores the loop's AI-response quality?
**Decision**: **Provisional — promptfoo as the backbone**, because it meets every criterion at the lowest cost (local CLI, config-driven, Anthropic-native, cheap, JSON output). Escalate to **Inspect AI** only if the evaluation unit (decided in [#120](https://github.com/johnalexwelch/dotdev/issues/120)) turns out to be full sandboxed agent task runs. **Harbor deferred** to a watch-list — best conceptual fit but Aug-2025-new and container-heavy; reassess in a later map pass. Final selection is gated on #120 (what gets evaluated), so this is a provisional pick, not a lock.
**Alternatives considered**: Harbor (rejected-for-now — immature/heavy despite best agent-eval fit); DeepEval (kept as the LLM-judge-metrics layer, ridable on either backbone); Braintrust / LangSmith / Arize Phoenix (rejected — hosted, violates no-vendor-lock); Coder Eval / OpenEval / AgentEval Lab (watch-list — young/narrow).
