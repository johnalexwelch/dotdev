...
- **Corpus**: a **private 20-50 task suite from real dotdev failures**, split **capability** (low pass rate, hill to climb) vs **regression** (~100%, catch backsliding); public benchmarks (SWE-bench, τ2-bench, etc.) are a separate model-selection concern, not the daily loop.
