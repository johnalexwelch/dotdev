...
## DL-0017 — Non-default-branch fallback auto-act asymmetry with drift-check #2

**Date**: 2026-07-22
**Context**: Issues #55, #56, #57; PR #54 (merge commit 8c45726) added the non-default-branch close fallback section to `reconcile-issues/SKILL.md`. A pi-rewind checkpoint commit (124c8b2) deleted the section body but left a dangling reference. This PR (#XX) restores the section and addresses follow-up hardening issues (#55 parameterize auth, #56 label code fences, #57 document interaction with drift-check #2).
**Question**: The fallback auto-closes issues (no approval gate) when a PR merges to a non-default branch and references the issue via Closes/Fixes/Resolves. Drift-check #2 (Issues closed incorrectly) detects when a closed issue's PR is reverted and proposes reopening (approval-gated). Should the fallback re-scan already-processed PRs and auto-reopen if a merge is reverted, or does that responsibility belong entirely to drift-check #2?
**Decision**: Asymmetry by design. The fallback auto-closes (no approval) on *newly observed* non-default-branch merges only; if that merge is later reverted, drift-check #2 (approval-gated) owns the decision to propose reopening. The fallback never re-scans already-processed PRs for status changes and treats a human reopen as authoritative — if a human has reopened a closed issue, the fallback will not auto-close it again on a future invocation. This separates concerns: the fallback is a one-time bookkeeping automaton that mirrors what GitHub would have done on the default branch; drift-check #2 handles follow-up decisions when context changes (reverts, evidence of continued work, etc.) and requires human judgment.
**Alternatives considered**:

- *Fallback also detects and auto-reopens reverted merges* — rejected: adds approval-bypass scope and requires the fallback to maintain state (which issues it previously closed, which PRs it saw) across invocations. Drift-check #2 is designed for this; let it own the decision.
- *All fallback closes are approval-gated like other drift fixes* — rejected: defeats the purpose. The narrow trigger (non-default-branch + new merge + Closes reference + OPEN issue) already makes false-positives unlikely; the merge itself was human-approved. Requiring approval for this bookkeeping defeats the efficiency gain of the fallback.
- *Fallback auto-reopens, drift-check #2 double-checks* — rejected: redundant state tracking and conflicting authorities. One clean hand-off is better.
**Tradeoffs accepted**: A reverted merge stays closed until drift-check #2 runs and proposes the reopen. This requires approving the reopen separately, which is asymmetric from the auto-close path. Acceptable because (a) reversions are rarer than forward merges, (b) human judgment is needed anyway ("should this really reopen, or was the merge wrong and we should land a new PR instead?"), and (c) the asymmetry is explicit and documented here rather than implicit in code.
**Follow-up**: Implement the `GH_PRIMARY_USER` env var (issue #55) to allow overriding the hardcoded default github account for auth-retry; the fallback section now documents this var per the environment-variable pattern in `docs/environment-variables.md`.

## DL-0018 — FIND-09 closure: no re-rewrite; local tag fix + GitHub Support caveat

**Date**: 2026-08-18
**Context**: FIND-09 re-reported by the 2026-07-28 skill-suite audit; remediation session 2026-08-18. Corrects and completes the dated entry "2026-07-09 - Erase the leaked ollama SSH key from git history (FIND-09)". Full evidence: `docs/audits/2026-08-18-find-09-remediation.md`.
**Question**: The 2026-07-09 entry recorded FIND-09 as erased (filter-repo + force-push, "private key = 0 objects on origin, commit 449613f GONE"), yet the 2026-07-20 and 2026-07-28 audits kept re-verifying the key as extractable. What was actually still exposed, and does closing FIND-09 require another history rewrite?
**Decision**: No new rewrite. The 2026-07-09 origin rewrite was real and held — a 2026-08-18 fresh clone contains neither commit `449613f47f994fca999ed8f3d659c41a17bc4b24` nor the private blob `3fef018335d31dc1cbd1323900f495925704ce0e`. The persistent "still open" signals came from two leftovers: (1) the primary checkout's **local `v0.1.0` tag** still pointed at the pre-rewrite line (`8c4572662cad9078a2949b4467f27affdab9c013`), keeping the tainted objects alive locally and making local `git show 449613f:…` evidence succeed — the 2026-07-20 audit's "no history rewrite" conclusion was wrong on this point; and (2) **GitHub server-side persistence**: 22 `refs/pull/*/head` refs (PRs 3–15, 41, 54, 58–62, 64, 67) still reach `449613f`, and the blob remains fetchable by SHA, incl. via the fork network (`lane-foxly/dotdev`). Actions: key re-confirmed dead (operator gate 1 — public half installed nowhere; no rotation needed); local tag re-pointed to origin's `e249f0e14753a03c3dfa6e0177f1f3126fbc56f3` (zero local refs now reach the tainted line; `reflog expire` + `gc --prune=now` handed to Alex — agent-blocked); GitHub Support contact documented as the **only** mechanism to purge PR refs, dereferenced objects, and cached views — Alex decides whether to file (draft in the remediation doc). FIND-09 closed as remediated.
**Alternatives considered**:

- Re-run `git filter-repo` (or BFG) + force-push per the original remediation brief — rejected: the clonable remote history is already clean; a rewrite would only remove the non-secret `.pub`, at the cost of changing every branch SHA, invalidating 5 open PRs, re-cutting ~24 worktrees, and dangling all D-006 ledger `head_sha` references. Because no rewrite ran, ledger head_shas stay valid.
